...
public class Rserve {
	...
	protected final static Logger LOG = LoggerFactory.getLogger(Rserve.class);
	 ...
	 */
	public synchronized static boolean launchRserve(String cmd, String rargs,
	...
	public static boolean checkLocalRserve() {
		...
		if (osname != null && osname.length() >= 7
			...
			try {
				...
				LOG.info("ERROR: unable to run REG to find the location of R: "
						+ rge);
				return false;
			}
			if (installPath == null) {
	...
	public static boolean isRserveRunning() {
		try {
			...
			LOG.debug("First connect try failed with: " + e.getMessage());
			return false;
		}
	}
	...
	public synchronized static boolean shutdownRserve() {
		...
		try {
			...
			LOG.debug("Rserve Already down: " + x.getMessage());
			return true;
		}
